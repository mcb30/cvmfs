...
void WritableCatalogManager::SwapNestedCatalog(const string &mountpoint,
  ...
  if (!FindCatalog(parent_path, &parent)) {
    PANIC(kLogStderr,
          "failed to find parent catalog '%s': "
          "mountpoint was not found in current catalog structure",
          parent_path.c_str());
  }
  ...
  shash::Any old_hash;
  uint64_t old_size;
  bool old_found = parent->FindNested(nested_root_ps, &old_hash, &old_size);
  if (!old_found) {
    PANIC(kLogStderr,
  ...
  if (!old_catalog) {
    PANIC(kLogStderr,
          "failed to swap nested catalog '%s': could not load old catalog",
          nested_root_path.c_str());
  }
  ...
  if (!new_catalog) {
    PANIC(kLogStderr,
          "failed to swap nested catalog '%s': could not load new catalog",
          nested_root_path.c_str());
  }
